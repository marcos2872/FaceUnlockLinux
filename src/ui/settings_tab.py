...
class SettingsTab(QWidget):
    def __init__(self, script_dir, parent=None):
        ...
        layout.addWidget(btn_save_config); layout.addStretch()
        self.setLayout(layout)
    ...
    def update_integration_checks(self):
        ...
        if not username:
            self.selected_user_label.setText("<b>Usuário Selecionado:</b> (Selecione na primeira aba)")
    ...
    def on_apply_integration(self):
        ...
        if QMessageBox.question(self, "Confirmar", f"Aplicar PAM para '{username}'?") == QMessageBox.Yes:
            services = {"sudo": self.check_sudo.isChecked(), 
